...
class TFModel(nn.Module):
    def __init__(self, img_size=64, patch_size=1, in_chans=3, out_chans=3, fuse_c=16,
        ...
        self.conv_after_body0 = nn.Sequential(nn.Conv2d(embed_dim*2, embed_dim, 3, 2, 1),
                                              nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                              nn.Conv2d(embed_dim, embed_dim, 3, 1, 1),
                                              nn.LeakyReLU(negative_slope=0.2, inplace=True),)
        self.conv_after_body1 = nn.Sequential(nn.Conv2d(embed_dim*4, embed_dim, 3, 2, 1),
                                              nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                              nn.Conv2d(embed_dim, embed_dim, 3, 1, 1),
                                              nn.LeakyReLU(negative_slope=0.2, inplace=True),)
        self.conv_after_body2 = nn.Sequential(nn.Conv2d(embed_dim*8, embed_dim, 3, 2, 1),
                                              nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                              nn.Conv2d(embed_dim, embed_dim, 3, 1, 1),
                                              nn.LeakyReLU(negative_slope=0.2, inplace=True),)
    ...
    def forward(self, x, c0, c1):
        ...
        fea1 = self.forward_features(torch.cat([s1, c0[0], c1[0]], dim=1), self.layers1)
        ...
        fea2 = self.forward_features(torch.cat([s2, c0[1], c1[1]], dim=1), self.layers2)
        ...
        fea3 = self.forward_features(torch.cat([s3, c0[2], c1[2]], dim=1), self.layers3)
