...
class FlowRefineNet_Multis_Simple(nn.Module):
    ...
    def forward(self, x0, x1, p):
        bs = x0.size(0)

        inp = torch.cat([x0, p, x1], dim=0)
        s_1 = self.conv1(inp)  # 1
        s_2 = self.conv2(s_1)  # 1/2
        s_3 = self.conv3(s_2)  # 1/4
        s_4 = self.conv4(s_3)  # 1/8
        ...
        # warp features by the updated flow
        c0 = [s_1[:bs], s_2[:bs], s_3[:bs], s_4[:bs]]
        po = [s_1[bs:2*bs], s_2[bs:2*bs], s_3[bs:2*bs], s_4[bs:2*bs]]
        c1 = [s_1[2*bs:], s_2[2*bs:], s_3[2*bs:], s_4[2*bs:]]
...
class VFIformerSmall(nn.Module):
    ...
    def forward(self, img0, img1, points):
        ...
        c0, c1, p = self.refinenet(img0, img1, points)
        ...
        i0 = self.cross_tran(p, c0)
        i1 = self.cross_tran(p, c1)
