...
class VFIformerSmall(nn.Module):
    def __init__(self, args):
        ...
        self.transformer = TFModel(img_size=(height, width), in_chans=2*c, out_chans=3, fuse_c=c,
                                          ...
                                          use_crossattn=[[[False, False, False, False], [False, False, False, False]], \
                                                      ...
                                                      [[False, False, False, False], [False, False, False, False]], \
                                                      [[False, False, False, False], [False, False, False, False]]])
        self.cross_tran = TFCModel(img_size=(height, width), in_chans=2*c, out_chans=3, fuse_c=c,
                                          window_size=window_size, img_range=1.,
                                          depths=[[3, 3], [3, 3], [3, 3], [1, 1]],
                                          embed_dim=embed_dim, num_heads=[[2, 2], [2, 2], [2, 2], [2, 2]], mlp_ratio=2,
                                          resi_connection='1conv')
    ...
    def forward(self, img0, img1, points, region_flow):
        ...
        fused_img0 = self.fuse_block1(torch.cat([warped_img0, warped_img2, img0], dim=1))
        fused_img1 = self.fuse_block2(torch.cat([warped_img1, warped_img3, img1], dim=1))
        ...
        x = self.fuse_block(torch.cat([warped_img0, warped_img1, points, warped_img2, warped_img3, img1, img0], dim=1))
