...
class VFIformerSmall(nn.Module):
    def __init__(self, args):
        ...
        self.cross = TFCModel(img_size=(height, width), in_chans=3, out_chans=4, fuse_c=embed_dim,
                                          window_size=window_size, img_range=1.,
                                          depths=[[3, 3], [3, 3], [2, 2], [1, 1]],
                                          embed_dim=embed_dim, num_heads=[[2, 2], [2, 2], [2, 2], [2, 2]], mlp_ratio=2,
                                          resi_connection='1conv',
    ...
    def forward(self, img0, img1, points):
        ...
        cross_output = self.cross(pred, points)
        cross_res = torch.sigmoid(cross_output[:, :3]) * 2 - 1
        cross_mask = torch.sigmoid(cross_output[:, 3:4])
        cross_merged_img = img0 * cross_mask + img1 * (1 - cross_mask)
        cross_pred = cross_merged_img + cross_res

        cross_pred = torch.clamp(cross_pred, 0, 1)
