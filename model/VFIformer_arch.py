...
class VFIformerSmall(nn.Module):
    def __init__(self, args):
        ...
        self.points_fuse = nn.Sequential(nn.Conv2d(3, 2*c, 3, 1, 1),
                                            nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                            nn.Conv2d(2*c, 3, 3, 1, 1),
                                            nn.LeakyReLU(negative_slope=0.2, inplace=True))
    ...
    def forward(self, img0, img1):
        ...
        x = self.fuse_block(torch.cat([warped_img0, warped_img1], dim=1))
        ...
        pred = self.final_fuse_block(torch.cat([res], dim=1))
