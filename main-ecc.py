...
if args.model == 'VFIT_S':
    ...
    from model.VFIformer_ecc import VFIformerSmall
...
def testt(args, epoch):
    ...
    t = time.time()
    with torch.no_grad():
        for i, (images, gt_image, datapath, flow, result) in enumerate(tqdm(test_loader)):

            images = [img_.to(device) for img_ in images]
            gt = gt_image.to(device)

            out = model(images[0], gt)
            out1 = model(images[0], result)
            out2 = model(gt, images[1])
            out3 = model(result, images[1])

            epe1 = torch.sum((out1 - out) ** 2, dim=1).sqrt()
            epe2 = torch.sum((out3 - out2) ** 2, dim=1).sqrt()
            # print(epe1.shape, epe2.shape)
            epe_list1.append(epe1.view(-1).cpu().numpy())
            epe_list2.append(epe2.view(-1).cpu().numpy())
...
def main(args):
    ...
    epe1, epe2 = testt(args, args.start_epoch)
    print("psnr :{}, ssim:{}".format(epe1, epe2))
    exit()
