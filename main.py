...
def train(args, epoch):
    ...
    for i, (images, gt_image, vector) in enumerate(train_loader):
        ...
        if args.model == 'VFI':
            with autocast():
                out = model(images[0], images[1], points, vector)
...
def test(args, epoch):
    ...
    t = time.time()
    with torch.no_grad():
        for i, (images, gt_image, datapath, vector) in enumerate(tqdm(test_loader)):
    ...
    with torch.no_grad():
        for i, (images, gt_image, datapath, vector) in enumerate(tqdm(test_loader)):
            ...
            for idx in range(out.size()[0]):
                ...
                # print(datapath[idx])
                os.makedirs(args.result_dir + '/' + datapath[idx])
                imwrite(out[idx], args.result_dir + '/' + datapath[idx] + '/abvector.png')
