...
class AnimeRun(Dataset):
    def __init__(self, data_root, is_training , input_frames="1357", mode='mini'):
        ...
        for d in dirs:
            ...
            data_list.append([img0, img1, points12, gt, d, region13, region31])
    ...
    def __getitem__(self, index):
        ...
        flow13 = np.load(self.data_list[index][5]).astype(np.float32)
        flow31 = np.load(self.data_list[index][6]).astype(np.float32)
        flow = [flow13, flow31]
        if self.training:
...
def get_loader(mode, data_root, batch_size, shuffle, num_workers, test_mode=None):
    if mode == 'train':
        ...
        is_training = False
    dataset = AnimeRun(data_root, is_training=is_training)
    return DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers, pin_memory=True)


if __name__ == "__main__":

    dataset = AnimeRun("/Users/shenjiaming/Documents/2022 S1/ELEC4712/MySuperGlue/atd12k_points", is_training=False)
    # print(dataset[0])
